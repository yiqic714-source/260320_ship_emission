...
NPZ_PATH = Path('/home/chenyiqi/260320_ship_emission/processed_data/sox_diff_2019_minus_2020_all_months.npz')
...
def build_step_weights(step_count: int, mode: str) -> np.ndarray:
	...
	for line_no in WEIGHT_SOURCE_LINES:
		...
		for token in parts[1:]:
			...
			if t == '':
				continue
			try:
				values.append(float(t)*100 - 100)
			except ValueError:
				continue
		if values:
			line_vectors.append(np.asarray(values, dtype=float))
	...
	min_len = min(vec.size for vec in line_vectors)
	if min_len <= 0:
		raise ValueError('Weight rows are empty after parsing.')

	stacked = np.vstack([vec[:min_len] for vec in line_vectors])
	avg_weights = np.mean(stacked, axis=0)

	if step_count > avg_weights.size:
		pad_value = float(avg_weights[-1])
		weights = np.pad(avg_weights, (0, step_count - avg_weights.size), mode='constant', constant_values=pad_value)
	else:
...
def plot_kept_t0_positions(
	...
	if kept_t0_records:
		t0_lons = [normalize_lon(lon) for _, _, lon in kept_t0_records]
		t0_lats = [lat for _, lat, _ in kept_t0_records]
		hours_since_first = [
			(record_dt - kept_t0_records[0][0]).total_seconds() / 3600.0
			for record_dt, _, _ in kept_t0_records
		]
		sc = ax.scatter(
			t0_lons,
			t0_lats,
		...
		cbar.set_label('Hours since first T0 UTC slot')
